...
import com.boozer.nexus.model.NexusTaskEntity;
import com.boozer.nexus.service.NexusTaskService;
...
public class NexusController {
    ...
    private final NexusOrchestrator nexusOrchestrator;
    
    @Autowired
    private NexusTaskService taskService;
    
    @Autowired
    public NexusController(NexusOrchestrator nexusOrchestrator) {
        this.nexusOrchestrator = nexusOrchestrator;
    }
    
    /**
     * Endpoint to submit a task to NEXUS
     */
    @PostMapping("/task")
    public ResponseEntity<TaskResponseDTO> submitTask(@RequestBody TaskRequestDTO taskDTO) {
        try {
            // Convert DTO to domain object
            NexusTask task = new NexusTask();
            ...
            NexusTaskEntity taskEntity = new NexusTaskEntity(
                taskId, 
            ...
            CompletableFuture<NexusResult> futureResult = nexusOrchestrator.executeTask(task);
    ...
    public ResponseEntity<TaskResponseDTO> triggerMaintenance() {
        try {
            ...
            NexusTask task = new NexusTask();
            task.setType(NexusTask.TaskType.AUTONOMOUS_MAINTENANCE);
    ...
    public ResponseEntity<NexusTaskEntity> getTaskStatus(@PathVariable String taskId) {
     ...
     */
    @GetMapping("/tasks")
    public ResponseEntity<List<NexusTaskEntity>> getAllTasks() {
    ...
    public ResponseEntity<Map<String, Object>> getSystemInfo() {
        ...
        response.put("name", "NEXUS AI System");
        response.put("version", "1.0.0");
        response.put("description", "Autonomous AI Engineer for Boozer Application");
        
        Map<String, Object> capabilities = new HashMap<>();
        capabilities.put("taskTypes", NexusTask.TaskType.values());
